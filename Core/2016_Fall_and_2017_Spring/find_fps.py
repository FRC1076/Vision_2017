...
grabbing = True
target_found = False
target_found_last_time = True;
target_img = np.zeros((1024,1024,3), np.uint8)
...
def DrawTarget(bShowTarget):
    # bShowTarget = True
    if bShowTarget:
        color = (0,80,0)
    else:
        color = (0,0,0)
    cv2.rectangle(target_img,(0,0),(200,500), color, cv2.FILLED)
    cv2.rectangle(target_img,(500,0),(700,500), color, cv2.FILLED)
...
while 1:
    try:
        ...
        # DrawTarget(target_found_last_time)
        if (target_found != target_found_last_time):
            DrawTarget(not target_found)
            target_found_last_time = target_found;
        #
